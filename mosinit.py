'''
Current clamp demo using Allen Brain's model 472451419.
...
from neuron472451419 import Neuron472451419
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472451419(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472451419; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-99.7316513062)
        h.fcurrent()
